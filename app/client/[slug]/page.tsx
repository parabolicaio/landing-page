...
function MilestoneTimeline({ milestones, color }: { milestones: Milestone[]; color: string }) {
  ...
  const W = 700; const H = 80; const PAD = 40;
  const trackW = W - PAD * 2; const trackY = 36; const nodeR = 9;

  function xOf(ts: number) { return PAD + ((ts - minTs) / range) * trackW; }

  const [tooltip, setTooltip] = useState<{ ms: Milestone; x: number } | null>(null);

  return (
    <div className="mb-6 overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[400px]" style={{ height: H }} onMouseLeave={() => setTooltip(null)}>
        <line x1={PAD} y1={trackY} x2={W - PAD} y2={trackY} stroke="#E5E7EB" strokeWidth={4} strokeLinecap="round" />
        <line x1={PAD} y1={trackY} x2={PAD + nowPct * trackW} y2={trackY} stroke={color} strokeWidth={4} strokeLinecap="round" />
        {sorted.map(ms => {
          const x = xOf(new Date(ms.due_date!).getTime());
          const dotColor = STATUS_DOT_COLORS[ms.status] || '#94A3B8';
          return (
            <g key={ms.id} onMouseEnter={() => setTooltip({ ms, x })} style={{ cursor: 'pointer' }}>
        ...
        {nowPct > 0 && nowPct < 1 && (
          <g>
            <line x1={PAD + nowPct * trackW} y1={trackY - 14} x2={PAD + nowPct * trackW} y2={trackY + 14} stroke={color} strokeWidth={1.5} strokeDasharray="3 2" />
            <text x={PAD + nowPct * trackW} y={trackY - 18} textAnchor="middle" fontSize={8} fill={color} fontWeight="600">Today</text>
          </g>
        )}
      </svg>
      {tooltip && (
        <div className="mt-1 bg-neutral-900 text-white text-xs rounded-lg px-3 py-2 shadow-lg inline-block"
          style={{ marginLeft: `${(tooltip.x / W) * 100}%`, transform: 'translateX(-50%)' }}>
          <p className="font-semibold">{tooltip.ms.title}</p>
          <p className="text-neutral-400">Due {new Date(tooltip.ms.due_date!).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
    ...
    return (
      <div className="min-h-screen bg-neutral-900 flex items-center justify-center p-4 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5"
          style={{ backgroundImage: `radial-gradient(circle at 1px 1px, #0FBAB0 1px, transparent 0)`, backgroundSize: '40px 40px' }}
        />
        ...
        <div className="relative z-10 w-full max-w-sm">
          ...
          <div className="bg-neutral-800 rounded-2xl border border-neutral-700 p-8 shadow-2xl">
            ...
            <div className={`flex justify-center gap-3 mb-6 ${shake ? 'animate-shake' : ''}`}>
              {pinDigits.map((d, i) => (
                <input
                  key={i} id={`pin-${i}`} type="password" inputMode="numeric" maxLength={1}
                  ...
                  className={`w-12 h-14 text-center text-xl font-bold rounded-xl border-2 bg-neutral-700 text-white transition-all focus:outline-none ${
                    d ? 'border-parabolica bg-parabolica/10 text-parabolica' : 'border-neutral-600 focus:border-parabolica'
                  }`}
                />
              ))}
            </div>
          ...
          <p className="text-center text-neutral-600 text-xs mt-6">Contact your project manager if you need access.</p>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="rounded-2xl p-8 mb-8 text-white relative overflow-hidden"
          ...
          <div className="relative z-10">
            <p className="text-white/70 text-sm font-medium uppercase tracking-widest mb-2">{projectInfo.client_name}</p>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm mt-0.5">{completed} of {milestones.length} milestones complete</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <div key={ms.id} className="bg-white rounded-xl border border-neutral-200 p-5 hover:shadow-md transition-all">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        {ms.description && <p className="text-sm text-neutral-500 mb-3 ml-5">{ms.description}</p>}
                        <div className="ml-5">
                      ...
                      {ms.due_date && (
                        <div className="shrink-0 text-right">
                          ...
                          <p className="text-sm font-semibold text-neutral-700">
                            {new Date(ms.due_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </p>
                        </div>
                      )}
                    </div>
